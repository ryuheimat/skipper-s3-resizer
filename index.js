...
module.exports = function SkipperS3 (globalOpts) {
  ...
  function S3Receiver (options) {
    ...
    receiver__._write = function onFile(__newFile, encoding, next) {
      ...
      var getFileName = _.isFunction(options.filename) ?
        options.filename
        : function (base, ext) {
      ...
      var ext = path.extname(__newFile.fd);
      var base = __newFile.fd.split(ext)[0];
      __newFile.fd = getFileName(base, ext);
      
      var width = _.isNumber(options.resize.width) ? 
        options.resize.width 
        : null;
      
      var height = _.isNumber(options.resize.height) ? 
        options.resize.height 
        : null;

      var stretch = options.resize.stretch || false;
      var __resizedFile;

      // If dimensions are not specified do not resize
      if (!width && !height)
        __resizedFile = __newFile;
      ...
      var mpu = new S3MultipartUpload({
        objectName: path.join(prefix, __newFile.fd), // File will be prefixed
        stream: __resizedFile,
